			...
			with(Concurrent) {
				...
				return function() {
					...
					function doNext() {
						...
						if(triplet.timeout < 0) { /* Do nothing. */ } else {
						...
						var ex_handler = {
							procedure: function(e) {
								if(e === LoadedException) {
									...
									try {
										req.abort();
									} catch(_) {} // IE less than 7 does not support "abort".
									return {
					...
					return function() {
						...
						function CzConvert(func) {
							...
							return new FunctionExpression(
								null, [var_this, var_args, var_cont],
								list(
									new ReturnStatement([],
										new CallExpression(
											new DotAccessor(body, name_apply), [var_this, var_args]
										)
									)
								)
							);
						...
						IL.EnumStatement.prototype[Cz] = function() {
							return new Block([], list(
								...
								new ForInStatement([], var_intermediate, this.exp,
									new ExpStatement([],
										new CallExpression(
											new DotAccessor(var_this, name_push), [var_intermediate]
										)
									)
								),
									...
									case Token.TRY:
										{
											...
											return finallyBlock ?
												new TryCatchListFinallyStatement(labels, tryBlock, catchList, finallyBlock, lineno, this.source) : new TryCatchListStatement(labels, tryBlock, catchList, lineno, this.source);
										}
